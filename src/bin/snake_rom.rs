...
// to load the rome provide that path as program argument e.g. "./src/bin/snake.nes"
fn main() {
    ...
    if args.len() > 1 {
        ...
        cpu = CPU::new_with_bus(Bus65k::new());
...
fn run_game(cpu: &mut CPU) {
    ...
    let window = video_subsystem
        ...
        .position_centered()
        .build().unwrap();
    let mut canves = window.into_canvas().present_vsync().build().unwrap();
    let mut event_pump = sdl_context.event_pump().unwrap();
    canves.set_scale(10.0, 10.0).unwrap();

    let creator = canves.texture_creator();
    let mut texture = creator.create_texture_target(PixelFormatEnum::RGB24, 32, 32).unwrap();

    let mut scree_state = [0u8; 32 * 3 * 32];
    ...
    cpu.run_with_callback(move |cpu| {
        ...
        if read_screen_state(cpu, &mut scree_state) {
            ...
            canves.copy(&texture, None, None).unwrap();
            canves.present();
        }
