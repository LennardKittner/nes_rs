...
impl ScanlinePixel {
    pub const fn new() -> ScanlinePixel {
        ScanlinePixel {
            background_color: BackgroundColor { color: (0, 0, 0), transparent: true },
            sprite_color: SpriteColor { color: (0, 0, 0), behind_background: false, transparent: true },
        }
    }
    
    pub fn get_combined_color(&self) -> (u8, u8, u8) {
        match (self.background_color.transparent, self.sprite_color.behind_background, self.sprite_color.transparent) {
